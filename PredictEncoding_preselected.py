...
from Helper import custom_loss
...
def train_naive_encoder(X_train, X_valid, Y_train, Y_valid, norms_X, norms_Y):  # Dan: I changed the name
...
def train_mod_autoencoders(X_train, X_valid, Y_train, Y_valid, norms_X, norms_Y):
...
def main():
    ...
    #select desired gene list here (needs to be provided in GeneLists.py)!
    #.CHOLESTASIS - 18 gene associated with cholestasis.
    #.NAFLD - 22 genes associated with NAFLD.
    #.STEATOSIS -  50 genes associated with steatosis.
    #.GTX_CAR - 76 genes associated with genotoxicity and carcinogenicity.
    gene_list = GeneLists.NAFLD  
    ...
    """select desired method under 'model'
    rf -> random forest
    mod_autoencoder -> modified autoencoder (with pre-training)
    naive_encoder -> naive encoder (no pre-training)
    cnn -> convolutional neural network
    """
    #train the specified model using leave-one-compound-out cross validation
    data = leave_one_out_evaluation(X, Y, data_compounds,model='naive_encoder',x_vivo_arg=x_vivo, y_vivo_arg=y_vivo)

    """ the following can be used to save the actual prediction values
    out_file_name = "file_name.mat"  # the actual prediction values will be written as at matlab workspace to file_name.mat
